...
use regex::Regex;
use syn::{
    parse_macro_input, FnArg, GenericParam, ImplItem, ImplItemMethod, ItemImpl, Stmt,
    WherePredicate,
};
        ...
        Some(function) => {
            ...
            if attr.to_string() == "false" {
                ...
                let async_trait_lifetime =
                    function.sig.generics.params.pop().unwrap().value().clone();
                function
                    .sig
                    .generics
                    .params
                    .push(parse_macro_input!(new_lifetime_token as GenericParam));
                function.sig.generics.params.push(async_trait_lifetime);
